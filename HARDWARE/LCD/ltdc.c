...
#else
	u16 ltdc_lcd_framebuf[1366][800] __attribute__((at(LCD_FRAME_BUF_ADDR)));	//定义最大屏分辨率时,LCD所需的帧缓存数组大小
#endif
...
u16 LTDC_PanelID_Read(void)
{
	...
	GPIO_Initure.Pin=GPIO_PIN_9|GPIO_PIN_10|GPIO_PIN_11|GPIO_PIN_12; //PH9,10,11,12
	...
	if(idx==7)                                     //如果是全1,有可能是没有接入任何设备,需要排除这个情况
	{
		 GPIO_Initure.Pin=GPIO_PIN_6;  //PG6
         GPIO_Initure.Mode=GPIO_MODE_INPUT;  //输入
		 ...
		 GPIO_Initure.Pin=GPIO_PIN_2|GPIO_PIN_7; //PI2,7下拉输入
         HAL_GPIO_Init(GPIOI,&GPIO_Initure);     //初始化
		
		 idx=(u8)HAL_GPIO_ReadPin(GPIOG,GPIO_PIN_6);    //读取M0
         idx|=(u8)HAL_GPIO_ReadPin(GPIOI,GPIO_PIN_2)<<1;//读取M1
         idx|=(u8)HAL_GPIO_ReadPin(GPIOI,GPIO_PIN_7)<<2;//读取M2
		  if(idx==0)idx=8;                            //上拉的时候全1,下拉的时候,全0,说明接入MS7024显示屏  
	}
	switch(idx)
	{
		...
		case 7:
		{
			vga_idx=HAL_GPIO_ReadPin(GPIOH,GPIO_PIN_9);//读取R3-KEY1
			vga_idx|=HAL_GPIO_ReadPin(GPIOH,GPIO_PIN_10)<<1;//读取R4-KEY2
			vga_idx|=HAL_GPIO_ReadPin(GPIOH,GPIO_PIN_11)<<2;//读取R5-KEY3
			vga_idx|=HAL_GPIO_ReadPin(GPIOH,GPIO_PIN_12)<<3;//读取R6-KEY4
		
			vga_idx=~vga_idx&0x0f;
			if(vga_idx==1) return 0XA001;//1366*768分辨率
			else if(vga_idx==2) return 0XA002;//1280*800分辨率
			else if(vga_idx==3) return 0XA003;//1280*768分辨率
			else if(vga_idx==4) return 0XA004;//1024*768分辨率
			else if(vga_idx==5) return 0XA005;//848*480分辨率
			else if(vga_idx==6) return 0XA006;//800*600分辨率
			else if(vga_idx==7) return 0XA007;//640*480分辨率
		}
		case 8:return 0x7024;
		default:return 0;
...
{   
	...
	lcdid=0x7024;			            //强制设置为MS7024的面板ID
	if(lcdid == 0x7024)
	{
		lcdltdc.pwidth=720;			    //面板宽度,单位:像素
		lcdltdc.pheight=480;		    //面板高度,单位:像素
		lcdltdc.hsw=62;				    //水平同步宽度62
		lcdltdc.vsw=6;				    //垂直同步宽度
		lcdltdc.hbp=60;				    //水平后廊60
		lcdltdc.vbp=30;				    //垂直后廊
		lcdltdc.hfp=16;			    //水平前廊16
		lcdltdc.vfp=9;				    //垂直前廊
    LTDC_Clk_Set(324,3,RCC_PLLSAIDIVR_4); //设置像素时钟 27M
	}
    ...
    }else if(lcdid==0X1018)			    //10.1寸1280*800 RGB屏	
	{
		...
		lcdltdc.vfp=10;				    //垂直前廊
		lcdltdc.vsw=3;				    //垂直同步宽度
		LTDC_Clk_Set(300,3,0); 		    //设置像素时钟  50Mhz   
	}
     ...
     { 
		...
		lcdltdc.vbp=24;				    //垂直后廊  
		lcdltdc.vfp=3;				    //垂直前廊
        LTDC_Clk_Set(114,2,RCC_PLLSAIDIVR_2);//设置像素时钟 28.5Mhz
     ...
     {  
		...
		lcdltdc.vbp=14;				    //垂直后廊 
		lcdltdc.vfp=3;				    //垂直前廊
		LTDC_Clk_Set(142,2,RCC_PLLSAIDIVR_2);//设置像素时钟 35.5Mhz
	 }else if(lcdid==0XA003)	        //接VGA显示器
	 {
		...
		lcdltdc.vbp=12;				    //垂直后廊
		lcdltdc.vfp=3;				    //垂直前廊
		LTDC_Clk_Set(137,2,RCC_PLLSAIDIVR_2);//设置像素时钟  34.25Mhz
	 }else if(lcdid==0XA004)            //接VGA显示器
	 {
		...
		lcdltdc.vbp=29;				    //垂直后廊 
		lcdltdc.vfp=3;				    //垂直前廊
		LTDC_Clk_Set(173,2,RCC_PLLSAIDIVR_2);//设置像素时钟 43.25Mhz	
	 }else if(lcdid==0XA005)            //接VGA显示器
	 {
		...
		lcdltdc.vbp=23;				    //垂直后廊
		lcdltdc.vfp=6;				    //垂直前廊
		LTDC_Clk_Set(68,2,RCC_PLLSAIDIVR_2);//设置像素时钟 17Mhz 
	 }else if(lcdid==0XA006)            //接VGA显示器
	 {
		...
		lcdltdc.vbp=23;				    //垂直后廊
		lcdltdc.vfp=1;				    //垂直前廊
		LTDC_Clk_Set(80,2,RCC_PLLSAIDIVR_2);//设置像素时钟 20mz		
	 }else if(lcdid==0XA007)            //接VGA显示器
	 {
		...
		lcdltdc.vbp=41;				    //垂直后廊
		lcdltdc.vfp=2;				    //垂直前廊
		LTDC_Clk_Set(51,2,RCC_PLLSAIDIVR_2);//设置像素时钟 12.75Mhz
	 }
